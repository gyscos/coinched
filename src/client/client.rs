...
impl <B: Backend, F: MainFrontend<B>> Client<B, F> {
    ...
    fn run_game(&mut self,
                first: pos::PlayerPos,
                hand: cards::Hand) -> Result<[i32; 2], GameError> {
    ...
    fn run_auction(&mut self,
                   mut auction: F::Auction) -> Result<<F::Auction as AuctionFrontend<B>>::Game, GameError> {
        loop {
            let mut event = self.backend.wait();
            match event {
                Ok(EventType::YourTurn) => event = match auction.ask_action() {
                    AuctionAction::Leave => {
            ...
            match event {
                ...
                Ok(EventType::BidOver(contract)) => {
                    return Ok(auction.auction_over(&contract))
                },
