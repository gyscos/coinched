        ...
        match self {
            ...
            &PlayerEvent::Coinched => {
                s.emit_struct("PlayerEvent",
                              1,
                              |s| s.emit_struct_field("type", 0, |s| "Coinched".encode(s)))
            }
            &PlayerEvent::Passed => {
                s.emit_struct("PlayerEvent",
                              1,
                              |s| s.emit_struct_field("type", 0, |s| "Passed".encode(s)))
            }
    ...
    fn encode<S: rustc_serialize::Encoder>(&self, s: &mut S) -> Result<(), S::Error> {
        match self {
            ...
            &EventType::BidCancelled => {
                s.emit_struct("Event",
                              1,
                              |s| s.emit_struct_field("type", 0, |s| "BidCancelled".encode(s)))
            }
            ...
            &EventType::TrickOver { winner: pos } => {
                s.emit_struct("Event", 2, |s| {
